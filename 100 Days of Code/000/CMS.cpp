/*
 * Awesome Source Code From https://code-projects.org/
 */
...
class student
{
	public:
		...
		void input()
		{
			...
			gets(name);
			cout<<"\n Enter roll no: ";
			cin>>reg;  // Long int variable in class student.
			fflush(stdin); // If the given stream was open for writing, any unwritten data in its output buffer is written to the file. If a null pointer, all such streams are flushed.
			cout<<"\n Enter Branch: ";
			gets(branch);
		}
		void display()
		{
			system("CLS"); // Screen Flush
...
void get_file() // Keyword user-defined name
{
	student x;
	int i=0;
	fstream f;
...
int main() // First to execute.
{
	int i=1;
	get_file(); // Defines
	while(i)
	{
		system("CLS"); // Flushes Screen
		cout<<"\t\t\t-----------------------------------------\n";
